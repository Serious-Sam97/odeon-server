...
O Odeon é dois. Eles conversam por HTTP e mais nada — sem tipo compartilhado,
sem código gerado, sem import cruzado.

| | | |
|---|---|---|
| **[odeon-server](https://github.com/Serious-Sam97/odeon-server)** | **você está aqui** | Rust · axum + sqlx + Postgres/pgvector · e o `docs/DESIGN.md` inteiro |
| **[odeon-client](https://github.com/Serious-Sam97/odeon-client)** | a interface | React + TypeScript, mais os clientes Kotlin de celular, TV e iOS |

**Este repositório é um servidor completo**: sobe, varre o disco, identifica,
transcodifica e serve `/api/...`. Qualquer coisa que fale HTTP consome ele — a
interface web é só a primeira delas.

O `src/api.ts` do cliente descreve essas respostas em TypeScript escrito à mão.
Ela já era uma cópia antes de os repositórios se separarem, e continua sendo: uma
rota que muda de forma aqui não tem mais nada que avise o outro lado. É a dívida
que a separação compra, e ela foi comprada sabendo.
...
**Não há serviço web aqui.** Suba o
[odeon-client](https://github.com/Serious-Sam97/odeon-client) — ou aponte
qualquer cliente pra esta API. Depois crie o administrador e vá na aba
**pastas** pra escolher onde estão suas mídias. Aí é **varrer**.
